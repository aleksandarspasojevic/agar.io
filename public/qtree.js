...
class QuadTree {
  ...
  insert(ob) {
    if (!this.boundary.contains(ob.pos)) {
      return false;
    }
    ...
    return (this.northeast.insert(ob) || this.northwest.insert(ob) ||
      this.southeast.insert(ob) || this.southwest.insert(ob));
  }
  ...
  query(range, found) {
    ...
    for(let i=0;i<this.players.length;i++){
      if (range.contains(this.players[i].pos)) {
